# -*- coding: utf-8 -*-
"""
FINAL validation script (Heuristic).
Reverts to the sqrt(2l+1) scaling which was empirically validated.
Rigorous coefficients (Step 853) led to massive over-binding (-1.70 Ha vs -0.78 Ha).
This suggests the Reference RHF may use a different coupling scheme or 
effective potential than pure 'Average of Configuration'.
"""
import numpy as np
...
print("HEURISTIC VALIDATION: Sqrt(2l+1) scaling for intra-shell d/f")
print("=" * 70)
...
CONFIGS = {
    ...
    'Cr': {'1s':2, '2s':2, '2p':6, '3s':2, '3p':6, '4s':1, '3d':5}, # Exception
...
for symbol, config in CONFIGS.items():
    ...
    for target_name in config.keys():
        ...
        for source_name, (source, R_j, P_j, n_j) in orbitals.items():
            ...
            # Standard BARE Exchange (Rigorous RHF)
            scale = 1.0
...
atoms_to_check = ["He", "Be", "C", "O", "Ne", "Mg", "Ar", "Cr", "Fe", "Co", "Ni", "Zn", "Kr"]
for s in atoms_to_check:
