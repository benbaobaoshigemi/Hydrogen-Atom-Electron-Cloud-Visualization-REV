# -*- coding: utf-8 -*-
"""
Debug Zn 3d orbital to find error source
"""
import numpy as np
import json, re
...
r = create_log_grid(r_min=1e-7, r_max=100.0, N=5000)  # Finer grid
...
# Analyze 3d
target_orb = '3d'
target, R_target, n_target, _ = orbitals[target_orb]
l_target = target.l
...
print(f"Zn 3d breakdown:")
print(f"  Reference epsilon(3d) = {atom_data['energies']['3d']:.6f} Ha")
print(f"  T = {T:.6f}, V_nuc = {V_nuc:.6f}")
print()
...
for other_name, (orb, R_j, n_j, _) in orbitals.items():
    ...
    print(f"  {other_name}: J = {J_contrib:.4f}, K = {K_contrib:.4f} (l={orb.l}, coeffs={coeffs})")
    total_J += J_contrib
    total_K += K_contrib
...
eps_ref = atom_data['energies']['3d']
print(f"  epsilon_calc = {eps_calc:.6f}")
print(f"  epsilon_ref  = {eps_ref:.6f}")
print(f"  Error = {100*(eps_calc - eps_ref)/abs(eps_ref):.2f}%")
