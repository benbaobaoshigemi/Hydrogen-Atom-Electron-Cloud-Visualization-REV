# -*- coding: utf-8 -*-
"""Test with FULL exchange coefficients"""
import numpy as np
...
# FULL coefficients: c_k = (2l'+1) * (2l+1) * (3j)^2
# But for exchange formula K = Σ_k c_k * G^k, we use:
# c_k = (2l_source+1) * (3j)^2 (for spherically averaged exchange)
EXCHANGE_COEFFS_FULL = {
    (0, 0): [(0, 1.0)],  # 1 * 1
    (0, 1): [(1, 1.0)],  # 3 * (1/3) = 1
    (1, 0): [(1, 1.0)],  # 1 * 1 = 1 (source is s)
    (1, 1): [(0, 1.0), (2, 2/5)],  # 3 * (1/3, 2/15) = (1, 2/5)
    (0, 2): [(2, 1.0)],  # 5 * (1/5) = 1
    (2, 0): [(2, 1.0)],  # 1 * 1 = 1 (source is s)
    (1, 2): [(1, 2/3), (3, 3/7)],  # 5 * (2/15, 3/35)
    (2, 1): [(1, 2/3), (3, 3/7)],  # 3 * (2/15, 3/35) = (2/5, 9/35)... wait
    (2, 2): [(0, 1.0), (2, 2/7), (4, 2/7)],  # 5 * (1/5, 2/35, 2/35) = (1, 2/7, 2/7)
}
...
print("TESTING FULL COEFFICIENTS")
print("=" * 70)
...
for symbol, config in CONFIGS.items():
    ...
    for target_name in config.keys():
        ...
        # Exchange with FULL coefficients: (n_j/2) * Σ_k c_k * G^k
        E_exchange = 0
        for source_name, (source, R_j, P_j, n_j) in orbitals.items():
            l_source = source.l
