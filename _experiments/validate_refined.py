# -*- coding: utf-8 -*-
"""
VALIDATION: Refined Exchange Scaling.
Distinguishes between Intra-shell and Inter-shell scaling.

Hypothesis: 
- d-d Intra-shell is anisotropic -> Scale 2.236 (sqrt(5))
- p-d Intra-shell (n=n') is standard -> Scale 1.0
- p-d Inter-shell (n!=n') is enhanced -> Scale 1.5 (screened/relaxed)
"""
import numpy as np
...
print("REFINED VALIDATION: Intra vs Inter Shell Scaling")
print("d-d intra: 2.236 | p-d inter: 1.5 | p-d intra: 1.0")
print("=" * 70)
...
for symbol, config in CONFIGS.items():
    ...
    for orb_name, occ in config.items():
        ...
        orb = Orbital(name=orb_name, l=get_l(orb_name), n_shell=get_n(orb_name), terms=terms)
    ...
    for target_name in config.keys():
        ...
        for source_name, (source, R_j, P_j, n_j) in orbitals.items():
            ...
            if l_source == l_target and l_source >= 2:
                # Always intra-shell for same orbital type in ground state
                # unless we have 3d and 4d? Zn/Kr only have 3d.
                scale = 2.236068 # sqrt(5)
            
            # p-d interaction
            if (l_target==1 and l_source==2) or (l_target==2 and l_source==1):
                if n_target_shell != n_source_shell:
                    scale = 1.5 # Inter-shell (e.g. 3d-4p, 3d-2p)
                else:
                    scale = 1.0 # Intra-shell (e.g. 3d-3p)
